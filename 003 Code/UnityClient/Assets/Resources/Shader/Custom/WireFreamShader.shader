Shader "Custom/WireFrameShader"
{
 Properties 
 {
  ...
  _WireThickness("Wire Thickness", Range(1.0, 10.0)) = 1.0
 }
 SubShader 
 {
   ...
   v2g vert(appdata_base v)
   {
       ...
       OUT.uv = v.texcoord; // the uv's aren't used in this shader but are included in case you want to use them
       return OUT;
   }
   ...
   {
       // distance of frag from triangle's center
       float d = min(IN.dist.x, min(IN.dist.y, IN.dist.z));
       // fade based on distance from center and wire thickness
       float I = exp2(-4.0 * d * d / (_WireThickness * _WireThickness));
