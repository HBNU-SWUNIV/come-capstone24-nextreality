...
namespace NextReality.Asset.Routine
{
    public class AssetDownTest : MonoBehaviour
    {
        ...
        public IEnumerator AssetSecurity(int mode, LoadTask downTask)
        {
            ...
            if (mode == 0) // 다운
            {
                yield return StartCoroutine(AssetDown(downTask, mode));
            }
            else if (mode == 1) // 메모리
            {
        ...
        protected IEnumerator AssetDown(LoadTask downTask, int mode)
        {
            int downFailCount = 0;
        ...
        public byte[] GetAsset(int mode, LoadTask downTask)
        {
            ...
            {
                return File.ReadAllBytes(fullFilePath + "_1").Concat(File.ReadAllBytes(fullFilePath + "_2")).ToArray();
            }
