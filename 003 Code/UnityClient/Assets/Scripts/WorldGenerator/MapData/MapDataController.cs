...
namespace NextReality.Asset
{
	public class MapDataController : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				GameObject.Destroy(MapDataController.instance.gameObject);
		...
		private void Update()
		{
			...
			else if (Input.GetKeyDown(KeyCode.M))
			{
				...
				StartCoroutine(MapLoad(0)); // 맵 불러오기
			}
		}
		...
		public IEnumerator MapSave()
		{
			...
			int obj_cnt = ObjectJsonFormat(mapObjDatas, map_id);
		...
		public int ObjectJsonFormat(List<MapObjectData> mapObjDatas, int map_id)
		{
			...
			foreach (GameObject obj in FindObjectsOfType<GameObject>())
			{
				if (obj.CompareTag("Object"))
				{
					...
					if (obj.GetComponentInChildren<Rigidbody>() != null) objData.isRigidbody = true;
					if (obj.GetComponentInChildren<MeshCollider>() != null) objData.isMeshCollider = true;

					objDatas.Add(objData);
				}
			}
			if (objDatas.Count == 0) // 맵에 오브젝트가 하나도 존재하지 않은 경우
			{
		...
		{
			GltfRoutineManager.Instance.RoutineInit(); // 에셋 Routine의 모든 작업을 취소
		...
		public IEnumerator ObjectLoad(List<MapObjectData> mapObjDatas)
		{
			...
			foreach (MapObjectData mapObjData in mapObjDatas)
			{
				foreach (ObjectData objData in mapObjData.objList)
				{
					...
					GltfRoutineManager.Instance.CreateObject(objData);
